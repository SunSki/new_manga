...
def to_json(title, link, date, site, img):
    json = {}
    json["title"] = title
    json["link"] = link
    json["date"] = date
            ...
            if title.text not in titles:
                if re.search('公開は終了しました', private.text) == None:
                    ...
                    if date_judge(date):
                        img = img.get('src')
                        json.append(
                            to_json(title.text, link.get('href'), date, 'plus', img))
        ...
        if len(results) > 1:
            ...
            if date_judge(date):
                ...
                if title not in title_list:
                    ...
                    json.append(
                        to_json(title, results[1], date, 'tonari', img))
        i += 1
        sleep(0.1)
...
def youngGet():
    ...
    for a in link_list:
        ...
        if date_judge(date):
            ...
            print(date)
            print(img)
            json.append(to_json(title, link, date, 'young', img))
            sleep(0.1)
    print('finish')
    sleep(1)
    return sort_date(json)
